...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*Estou a fazer porra, se n�o for em 2 minutos tenta de novo*',
				success: '✓Sucesso✓',
				...
				only: {
					...
					ownerG: '🔒 Este comando só pode ser usado pelo proprietario do grupo, membro comum! 🔒',
					ownerB: '⚠️ Este comando só pode ser usado pelo meu dono ⚠️',
					admin: 'Este comando só pode ser usado por administradores do grupo membro comum! ',
					Badmin: '⚠️ Este comando só pode ser usado quando o bot se torna administrador né burrão! ⚠️',
                                        daftarB: `⚠️REGISTRE-SE⚠️\nOlá mano(a)!\nVocê não está registrado no banco de dados, para se registrar siga o exemplo abaixo:\n\nComando : ${prefix}registrar nome|idade\nExemplo : ${prefix}registrar Hidra|18`,
				}
			}
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '556796005372@s.whatsapp.net'
                        /******ApiKey Input******/
               ...
               break
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recebido, KKKK otário perdeu adm se fodeu @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pronto carai adicionado como administrador : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'creator':
                  client.sendMessage(from, {displayname: "Vitim", vcard: vcard}, MessageType.contact, { quoted: mek})
               client.sendMessage(from, 'Aqui está o número do meu dono, o mais brabo salve depois😳',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
					...
					try {
						...
						reply('usuario invalido, cê usa TikTok🤮')
					}
					break
				case 'snowwrite':
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'ytmp3':
					...
					teks = `📍 *Título* : ${anu.title}\nâ *Tamanho* : ${anu.filesize}\n\Espera ai man o áudio está sendo enviado...`
				...
				case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					teks = `*♛ Título* : ${anu.title}\n\n*VÍDEO ESTÁ ENVIANDO, NÃO SPAME SE NÃO FOR, FAVOR TENTAR NOVAMENTE*`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
	...
	case 'kbbi':
	      ...
	      tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'audio':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual é a sigla de idioma?', text, {quoted: mek})
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('O texto ta muito grande zé')
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('🔔 Mencionando Todos 🔔'+teks+'📜', members_id, true)
					break
                case 'marcar2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '🔔 Mencionando Todos 🔔'+teks+'📜', text, {quoted: mek})
					break
                case 'marcar3':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '🔔 Mencionando Todos 🔔'+teks+'📜', text, {detectLinks: false, quoted: mek})
					break
                        case 'marcar4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'tm':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*⚠️ TRANSMISSAO ⚠️*\n\n${body.slice(4)}`)
						}
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*⚠️ BC GROUP ⚠️*\n*Grupo* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Tchauzinhoo filha da puta🤡', text) // ur cods
					}, 0)
                     break
			...
			case 'igstalk':
               ...
               client.updatePresence(from, Presence.composing) 
              options = {
          text: `O dono do grupo é macaco do : @${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('A quem você deseja adicionar mano??')
	    ...
	    case 'kapankah':
			...
			case 'fechargp':
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedSticker) return reply('⚠️ Apenas Figurinhas ⚠️')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('⚠️ Só figurinha parada mano ⚠️')
                	...
                	client.updatePresence(from, Presence.composing) 
                        if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('Só funciona com video, ou puxa o video e tenta de novo')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
                ...
                break
                   ...
                   break
					...
					if (Number(args[0]) === 1) {
						...
						reply('✔️ SUCESSO boas ativo, agora vou manda foto da macacada😳')
            ...
            break
					...
					case 'nsfwbobs': 
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Tai os peitos seu corno\njá vai pro banheiro KKK'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('⚠️ *ERRO* ⚠️')
						}
					...
					case 'nsfwblowjob':
						try {
							...
							buffer = await getBuffer(res.result)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Partiu banheiro né?'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('⚠️ *ERRO* ⚠️')
						}
					...
					case 'trap':
						try {
							...
							buffer = await getBuffer(res.result)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nianji m'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('⚠️ *ERRO* ⚠️')
						}
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Bixa inutil ativada com sucesso ✔️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
				...
				case 'clone':
					...
					try {
						...
						mentions(`Foto de perfil do bot foi trocada, agora esta a desse ser humano: @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('Falha, ou o ser marcado não usa foto')
					}
					break
                                /*case 'magernulis':
                                        var gh = body.slice(11)
